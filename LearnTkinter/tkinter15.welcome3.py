...
image_file = tk.PhotoImage(
    file='/home/ohh/ohhPythonProject/LearnTkinter/welcome.gif')
# image_file = tk.PhotoImage(file='welcome.gif')
image = canvas.create_image(0, 0, anchor='nw', image=image_file)
canvas.pack(side='top')

tk.Label(window, text='User name: ').place(x=50, y=150)
tk.Label(window, text='User password: ').place(x=50, y=190)
...
def usr_login():
    ...
    if usr_name in usrs_info:
        ...
        is_sign_up = tk.messagebox.askyesno(
            title='Welcome', message='You havn\'t sign up yet.Sign up today?')
        if is_sign_up:
            usr_sign_up()


def usr_sign_up():
    def sign_to_window_python():
        np = new_pwd.get()
